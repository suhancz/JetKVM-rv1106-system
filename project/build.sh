...
function __RELEASE_FILESYSTEM_FILES()
{
	...
	if [ "$(id -u)" -eq 0 ]; then
		msg_info "current user is root, no need to change owner"
	else
		ls -la "$_target_dir/"
		stat "$_target_dir"
		msg_info "fixing permission for $_target_dir"
...
EOF
	...
	cat > $RK_PROJECT_PACKAGE_ROOTFS_DIR/etc/init.d/S10splash  << EOF
#!/bin/sh
cat /oem/usr/share/fb_init > /dev/fb0
EOF
	chmod a+x $RK_PROJECT_PACKAGE_ROOTFS_DIR/etc/init.d/S10splash
}
...
function __GET_TARGET_PARTITION_FS_TYPE()
{
	...
	do
		...
		if [[ $part_name == "system_a" ]]; then
			__MAKE_MOUNT_SCRIPT "mount_part system_b $part_mountpoint $part_fs_type ;"
		fi
	done
	IFS=
